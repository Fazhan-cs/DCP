...
parser.add_argument('--root_path', type=str, default='/data/fazhan/BCP-main/code/data', help='Name of Dataset')
...
def get_cut_mask_ens(out1, out2, nms=0):
    ...
    if nms == 1:
        masks = LargestCC_pancreas(masks)
    return masks
def get_cut_mask_ex(out1, out2, thres=0.5, nms=0):
    probs1 = F.softmax(out1, 1)
    probs2 = F.softmax(out2, 1)
    prob=probs1+probs2
    masks = (prob >= 1).type(torch.int64) 
    masks = masks[:, 1, :, :].contiguous()
    if nms == 1:
        masks = LargestCC_pancreas(masks)
    return masks
...
def get_cut_mask_exx(out1, out2, thres=0.5, nms=0):
    ...
    foreground1 = output1[:, 1, ...]
    foreground2 = output2[:, 1, ...]
    # 计算概率分布差异
    diff = torch.abs(foreground1 - foreground2)
    print(torch.mean(diff))
    pseudolabel=torch.zeros_like(output1)
    # 判断概率分布差异是否大于阈值
    if torch.mean(diff) > 0.005:
            # 融合两个网络的优势
            fused_output = (output1 + output2) / 2.0
            pseudolabel = torch.argmax(fused_output, axis=1)
            
    else:
            # 选择置信度较高的结果作为伪标签
            confidence1 = torch.max(output1, axis=1)[0]
            confidence2 = torch.max(output2, axis=1)[0]            
            pseudolabel = torch.where(confidence1 > confidence2, torch.argmax(output1, axis=1), torch.argmax(output2, axis=1))
            
    pseudolabel = pseudolabel.contiguous()
    if nms == 1:
        pseudolabel = LargestCC_pancreas(pseudolabel)
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    ema_model_copy = net_factory(net_type=args.model, in_chns=1, class_num=num_classes, mode="train")
    ...
    load_net(ema_model_copy, pretrained_model)
    
    
    model.train()
    ema_model.train()
    ema_model_copy.train()
    ...
    for epoch in iterator:
        ...
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                unoutput_a_ex, _ = ema_model_copy(unimg_a)
                unoutput_b_ex, _ = ema_model_copy(unimg_b)
                
                
                #probs1 = F.softmax(unoutput_a, 1)
                #probs2 = F.softmax(unoutput_a_ex, 1)
                
                #prob_diff = torch.abs(probs1 - probs2)
                #diff=torch.mean(prob_diff)
                
                
                plab_a = get_cut_mask_ens(unoutput_a,unoutput_a_ex, nms=1)
                plab_b = get_cut_mask_ens(unoutput_b,unoutput_b_ex, nms=1)

                img_mask_a, loss_mask_a = context_mask(img_a, 1/3)
                img_mask_b, loss_mask_b = context_mask(img_b, 1/3)
                img_mask, loss_mask = context_mask(img_b, args.mask_ratio)
            ...
            if flag==0:
                    ...
                    mixla_lab = lab_a * img_mask_a + plab_a * (1 - img_mask_a)
                    mixla_lab_x = plab_a * img_mask_a + lab_a * (1 - img_mask_a)
            ...
            #if flag==0 and np.random.random()>confidence:
            if flag==1:
                ...
                mixlb_lab = lab_b * img_mask_b + plab_b * (1 - img_mask_b)
                mixlb_lab_x = plab_b * img_mask_b + lab_b * (1 - img_mask_b)
            ...
            mixl_lab = mixla_lab * img_mask + mixlb_lab_x * (1 - img_mask)
            mixu_lab = mixla_lab_x * img_mask + mixlb_lab * (1 - img_mask)
            ...
            loss_l = mix_loss(outputs_l, mixla_lab, mixlb_lab_x, loss_mask_ex, u_weight=args.u_weight)
            loss_u = mix_loss(outputs_u, mixla_lab_x, mixlb_lab, loss_mask_ex, u_weight=args.u_weight,unlab=True)
            #loss_u=(loss_dice+loss_ce)/2
            loss = loss_l + loss_u
            ...
            writer.add_scalar('Self/diff', diff, iter_num)
            ...
            if flag==1:
                ...
                update_ema_variables(model, ema_model_copy, 0.99)
            ...
            if iter_num % 200 == 0:
                ...
                if dice_sample > best_dice:
                    ...
                    save_ema = os.path.join(self_snapshot_path,'ema_best_model.pth')
                    save_ema_copy = os.path.join(self_snapshot_path,'ema_copy_best_model.pth')
                    ...
                    torch.save(model.state_dict(), save_mode_path)
                    torch.save(model.state_dict(), save_best_path)
                    torch.save(ema_model.state_dict(), save_ema)
                    torch.save(ema_model_copy.state_dict(), save_ema_copy)
            ...
            if iter_num >= self_max_iterations:
                break
        writer.add_scalar('Self/a', a, iter_num)
        if iter_num >= self_max_iterations:
            iterator.close()
            break
    writer.close()
...
if __name__ == "__main__":
    ...
    self_snapshot_path = "./model/BCP/LA_{}_{}_labeled/self_train_ens0.005".format(args.exp, args.labelnum)
    ...
    shutil.copy('../code/LA_BCP_train_m.py', self_snapshot_path)
    # -- Pre-Training
