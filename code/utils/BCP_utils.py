...
def interval_mask(img, mask_ratio, interval=20):
    ...
    patch_pixel_x=patch_pixel_y=patch_pixel_z=int(interval/2)
    # 生成间隔型掩码
    for i in range(0, img_x, interval):
        for j in range(0, img_y, interval):
            for k in range(0, img_z, interval):
                if i+patch_pixel_x<112 and j+patch_pixel_y<112 and k+patch_pixel_z<80:
                    mask[i:i+patch_pixel_x, j:j+patch_pixel_y, k:k+patch_pixel_z] = 0
                    loss_mask[:, i:i+patch_pixel_x, j:j+patch_pixel_y, k:k+patch_pixel_z] = 0

    return mask.long(), loss_mask.long()


def ball_mask(img, mask_ratio):
    batch_size, channel, img_x, img_y, img_z = img.shape
    loss_mask = torch.ones(batch_size, img_x, img_y, img_z).cuda()
    mask = torch.ones(img_x, img_y, img_z).cuda()

    patch_radius_x = int(img_x * mask_ratio / 2)
    patch_radius_y = int(img_y * mask_ratio / 2)
    patch_radius_z = int(img_z * mask_ratio / 2)

    center_x = np.random.randint(patch_radius_x, img_x - patch_radius_x)
    center_y = np.random.randint(patch_radius_y, img_y - patch_radius_y)
    center_z = np.random.randint(patch_radius_z, img_z - patch_radius_z)

    grid = np.mgrid[0:img_x, 0:img_y, 0:img_z]
    grid = np.moveaxis(grid, 0, -1)
    distances = np.linalg.norm(grid - [center_x, center_y, center_z], axis=-1)

    mask[distances < patch_radius_x] = 0
    loss_mask[:, distances < patch_radius_x] = 0
...
def context_mask1(img, mask_ratio):
    batch_size, channel, img_x, img_y, img_z = img.shape[0],img.shape[1],img.shape[2],img.shape[3],img.shape[4]
    loss_mask = torch.ones(batch_size, img_x, img_y, img_z).cuda()
    mask = torch.ones(img_x, img_y, img_z).cuda()
    patch_pixel_x, patch_pixel_y, patch_pixel_z = int(img_x*mask_ratio), int(img_y*mask_ratio), int(img_z*mask_ratio)
    ...
    h = np.random.randint(0, 112 - patch_pixel_y)
    z = np.random.randint(0, 80 - patch_pixel_z)
    mask[h:h+patch_pixel_x, w:w+patch_pixel_y, z:z+patch_pixel_z] = 0
    loss_mask[:, h:h+patch_pixel_x, w:w+patch_pixel_y, z:z+patch_pixel_z] = 0
    return mask.long(), loss_mask.long()
...
def fix_loss(net3_output, img_l, patch_l, mask, l_weight=1.0, u_weight=0.5, unlab=False):
    img_l, patch_l = img_l.type(torch.int64), patch_l.type(torch.int64)
    image_weight, patch_weight = l_weight, u_weight
    if unlab:
        image_weight, patch_weight = u_weight, l_weight
