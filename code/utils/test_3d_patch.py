...
def var_all_case_LA_2D(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
   
    with open('/data/fazhan/BCP-main/code/data/2018LA_Seg_Training Set/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = ["/data/fazhan/BCP-main/code/data/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
    ...
    for image_path in loader:
        ...
        image = h5f['image'][:]
        label = h5f['label'][:]
        prediction, score_map = test_single_case_2D(model, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
...
def test_all_case_2d(model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
...
def test_single_case_2D(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    B, C, H, W, D = test_patch.shape
                    img_train_sup_2D = test_patch.permute(0, 4, 1, 2, 3).reshape(B*D, C, H, W)
                    #检验一下输入大小
                    
                    y1 = model(img_train_sup_2D)
                    
                    y1 = y1.reshape(B, D,2, H, W).permute(0, 2, 3, 4, 1)
                    y = F.softmax(y1, dim=1)
                    # print(y1[0,1,50,50,50])
                    # print(y1[0,0,50,50,50])
                    # print(y[0,1,50,50,50])
                    # print(y[0,0,50,50,50])
                    # image1=(y[0,1,:,:,50]>0.5).cpu().detach().numpy()
                    # image2=test_patch[0,0,:,:,50].cpu().detach().numpy()
