...
def get_dataset_path(dataset='pancreas', labelp='20percent'):
    ...
    return ['/'.join(['/data/fazhan/BCP-main/code/data/pancreas/data_lists', dataset, labelp, f]) for f in files]
...
class Pancreas(Dataset):
    ...
    def __init__(self, base_dir, name, split, no_crop=False, labelp=10, reverse=False, TTA=False):
        ...
        if split == 'train_lab':
            ...
            data_path = "/data/fazhan/BCP-main/code/data/pancreas/data_lists/pancreas/20percent/test.txt"
...
def get_ema_model_and_dataloader_ex(data_root, split_name, batch_size, lr, labelp=10):
    print("Initialize ema cutmix: network, optimizer and datasets...")
    """Net & optimizer"""
    net = create_Vnet()
    ema_net = create_Vnet(ema=True).cuda()
    ema_net_copy = create_Vnet(ema=True).cuda()
    ...
    testset = Pancreas(data_root, split_name, split='test')
    test_loader = DataLoader(testset, batch_size=1, shuffle=False, num_workers=0)
    return net, ema_net,ema_net_copy, optimizer, lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b, test_loader