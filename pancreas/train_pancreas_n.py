...
pretraining_epochs, self_training_epochs = 120, 500
...
def ema_cutmix_ex(net, ema_net,ema_net_copy, optimizer, lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b, test_loader):
    """Create Path"""
    save_path = Path(result_dir) / 'self_train_>1mask'
    save_path.mkdir(exist_ok=True)
    
    shutil.copy('/data/fazhan/BCP-main/code/pancreas/train_pancreas_n.py', save_path)
    ...
    load_net_opt(net, optimizer, pretrained_path / f'best_ema{label_percent}_pre.pth')
    load_net_opt(ema_net, optimizer, pretrained_path / f'best_ema{label_percent}_pre.pth')
    load_net_opt(ema_net_copy, optimizer, pretrained_path / f'best_ema{label_percent}_pre.pth')
    ...
    for epoch in tqdm_load(range(1, self_training_epochs+1)):
        ...
        for step, ((img_a, lab_a), (img_b, lab_b), (unimg_a, unlab_a), (unimg_b, unlab_b)) in enumerate(zip(lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b)):
            ...
            with torch.no_grad():
                ...
                uimg_a_plab = get_cut_mask_ex(unimg_a_out,unimg_a_out_ex, nms=True, connect_mode=connect_mode)
                uimg_b_plab = get_cut_mask_ex(unimg_b_out,unimg_b_out_ex, nms=True, connect_mode=connect_mode)
                img_mask, loss_mask = generate_mask(img_a, patch_size)  
...
def test_model(net, test_loader):
    ...
    load_path='/data/fazhan/BCP-main/code/pancreas/result/cutmix/self_train/best_ema_0.8393_epoch240_self.pth'
