...
def get_cut_mask(out, thres=0.5, nms=True, connect_mode=1):
    ...
    return masks
def get_cut_mask_ex(out1,out2, thres=0.5, nms=True, connect_mode=1):
    
    probs1 = F.softmax(out1, 1)
    probs2 = F.softmax(out2, 1)

    probs=probs1+probs2
    masks = (probs >= 1).type(torch.int64)
...
def get_cut_mask_exx(out1,out2, thres=0.5, nms=True, connect_mode=1):
    ...
    probs1 = F.softmax(out1, 1)
    probs2 = F.softmax(out2, 1)
    masks1 = (probs1 >= thres).type(torch.int64)
    masks2 = (probs2 >= thres).type(torch.int64)
    masks = (masks1 & masks2)
    masks = masks[:, 1, :, :].contiguous()
    if nms == 1:
...
def get_cut_mask_ens(out1, out2, thres=0.5, nms=True, connect_mode=1):
    probs1 = F.softmax(out1, 1)
    probs2 = F.softmax(out2, 1)
    prob_sum = probs1 + probs2
    prob_diff = torch.abs(probs1 - probs2)
    print(torch.mean(prob_diff))
    if torch.mean(prob_diff) > 0.001:
